...
if (!empty($first_route[1])) {
  $gets = explode('&', $first_route[1]);
  foreach ($gets as $get) {
    $get = explode('=', $get);
    if (!empty($get[0])) $_GET[$get[0]] = $get[1] ?? '';
  }
}
...
if (!empty($_GET["lang"]) && ($user['auth'] ?? 0) != 1):
  ...
  if (!empty($_SESSION["lang"]) && ($user['auth'] ?? 0) != 1):
    $language = $_SESSION["lang"];
  elseif (($user['auth'] ?? 0) != 1):
...
if (!isset($route[0]) && ($_SESSION["toutlike_userlogin"] ?? false) == true) {
  $route[0] = "neworder";
  $routeType = 0;
} elseif (!isset($route[0]) && ($_SESSION["toutlike_userlogin"] ?? false) == false) {
  $route[0] = "auth";
  $routeType = 1;
} elseif (($route[0] ?? '') == "auth" && ($_SESSION["toutlike_userlogin"] ?? false) == false) {
  $routeType = 1;
} else {
  $routeType = 0;
}
...
$link = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ?
  ...
  $_SERVER['REQUEST_URI'];
$pageurl = $link;
// [SECURITE] Phone-home BD Lab supprime (auto_register_site) — ToutLike Phase 1
...
// [SECURITE] Rent control BD Lab supprime (paidRent) — ToutLike Phase 1
...
// [SECURITE] Phase 1 — Protection CSRF sur tous les POST (sauf API/payment)
csrf_protect();

// [PHASE 2] Middleware auth — vérifie les permissions via la table de routes
$route_table = require __DIR__ . '/app/routes.php';
$current_route = $route[0] ?? '404';
if (isset($route_table[$current_route]) && $route_table[$current_route]['auth'] === true) {
    if (($_SESSION['toutlike_userlogin'] ?? 0) != 1 && $current_route !== 'admin') {
...
if ($settings["recaptcha"] == 1) {
  $captcha = false;
} elseif (($settings["recaptcha"] == 2 && (route(0) == "signup" || route(0) == "resetpassword")) || ($_SESSION["recaptcha"] ?? false)) {
  $captcha = true;
}
...
if (route(0) != "admin" && route(0) != "ajax_data") {
  ...
  foreach ($languages as $language) {
    ...
    if (!empty($_SESSION["lang"]) && $language["language_code"] == $_SESSION["lang"]) {
      $l["active"] = 1;
  ...
  if (route(0) != 'admin' && route(1) != 'v2' && route(0) != 'ajax_data') {
    ...
    if ($_SESSION['toutlike_userlogin'] != 1) {
