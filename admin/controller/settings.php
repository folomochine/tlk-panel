...
elseif (route(2) == "general"):
  ...
  if ($access):
    if ($_POST):
      ...
      if (!empty($_FILES["logo"]["tmp_name"]) && toutlike_upload_is_allowed_image($_FILES["logo"])):
      ...
      if (!empty($_FILES["favicon"]["tmp_name"]) && toutlike_upload_is_allowed_image($_FILES["favicon"])):
...
email_confirmation=:email_confirmation,
    ...
    elseif (route(3) == "delete-favicon"):
      ...
      if ($update):
        ...
        if (!empty($faviconPath) && strpos($faviconPath, "http") === 0) {
          $parsedPath = parse_url($faviconPath, PHP_URL_PATH);
          if (!empty($parsedPath)) {
            $faviconPath = ltrim($parsedPath, "/");
          }
        }
        if (!empty($faviconPath) && file_exists($faviconPath)) {
          unlink($faviconPath);
        }
      endif;
      header("Location:" . site_url("admin/settings/general"));
