...
if( $mode == 2 && $api["api_type"] == 1 ):
  ...
  $__svcLine = $conn->prepare("SELECT * FROM services WHERE category_id = :cat ORDER BY service_line DESC LIMIT 1");
$__svcLine->execute(["cat" => $category]);
$row = $__svcLine->fetch(PDO::FETCH_ASSOC);
...
$detail="";
  ...
  if( $serviceInfo["category_id"] != $category ): $__svcLine = $conn->prepare("SELECT * FROM services WHERE category_id = :cat ORDER BY service_line DESC LIMIT 1");
$__svcLine->execute(["cat" => $category]);
$row = $__svcLine->fetch(PDO::FETCH_ASSOC); $last_category=$serviceInfo["category_id"]; $last_line=$serviceInfo["service_line"]; $line= $row["service_line"] + 1; else: $line= $serviceInfo["service_line"]; endif;
  $conn->beginTransaction();
