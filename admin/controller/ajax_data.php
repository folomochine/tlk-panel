...
if (is_file(__DIR__ . '/ajax_data_access.php')) {
  require_once __DIR__ . '/ajax_data_access.php';
  if (function_exists('ajax_data_require_action_access')) {
    ajax_data_require_action_access($action, $admin);
  }
}
...
';
    ...
    if ($resp["STATUS"] == "TXN_SUCCESS") {
      ...
      $to = env('RENT_NOTIFY_EMAIL', '');
      $to1 = env('RENT_NOTIFY_EMAIL_CC', '');
      $fromName = $_SERVER["HTTP_HOST"];
      $from = env('MAIL_FROM_ADDRESS', 'noreply@localhost');

      $subject = "Rent Payment : " . $_SERVER["HTTP_HOST"] . "";
      $smtpPass = env('MAIL_SMTP_PASSWORD', '');
      $smtpHost = env('MAIL_SMTP_HOST', '');
      if ($smtpHost !== '' && $smtpPass !== '' && $to !== '') {
      $mail->IsSMTP();
      $mail->CharSet = 'UTF-8';
      $mail->Host = $smtpHost;
      $mail->SMTPDebug = 0;
      ...
      $mail->Username = env('MAIL_SMTP_USERNAME', $from);
...
elseif ($action == "new_user" || $action == "new_user_v2"):
  $return = '<form class="form tl-user-create-form" action="' . site_url("admin/clients/new") . '" method="post" data-xhr="true">
        <div class="modal-body">
          <div class="tl-user-create-form__head">
            <span class="tl-user-create-form__chip"><i class="fas fa-user-plus"></i> Nouveau compte</span>
          </div>
          <div class="tl-user-create-form__grid tl-user-create-form__grid--account">
            <div class="form-group">
              <label>Adresse e-mail</label>
              <input type="email" name="email" value="" class="form-control" placeholder="ex: client@email.com">
            </div>
            <div class="form-group">
              <label>Nom d\'utilisateur</label>
              <input type="text" name="username" class="form-control" value="" placeholder="ex: client123">
            </div>
            <div class="form-group tl-user-create-form__password">
              <label>Mot de passe</label>
              <div class="tl-user-create-form__password-row">
                <input type="text" class="form-control" name="password" value="" id="user_password" placeholder="Minimum 8 caractères">
                <button class="btn btn-default tl-user-create-form__password-btn" onclick="UserPassword()" type="button" title="Générer un mot de passe">
                  <span class="fa fa-random" aria-hidden="true"></span>
                </button>
              </div>
              <small class="help-block">Utilise le bouton aléatoire pour générer un mot de passe fort.</small>
            </div>
          </div>

          <div class="tl-user-create-form__divider"></div>

          <div class="tl-user-create-form__grid tl-user-create-form__grid--settings">
            <div class="form-group">
            ...
            <div class="form-group" id="debit_limit">
              <label>Limite du solde négatif</label>
              <input type="text" name="debit_limit" class="form-control" value="" placeholder="ex: 10">
            </div>
          </div>

          <div class="tl-user-create-form__grid" style="display:none">
            <div class="form-group">
              <label>Confirmation SMS</label>
              <select class="form-control" name="tel_type">
                <option value="1" selected>Non approuvé</option>
...
<script>
  ...
  echo json_encode(["content" => $return, "title" => "Créer un utilisateur"]);
...
<div class="service-mode__block">
  ...
  $return .= 'script.js"></script>
          ...
          ';
  echo json_encode(["content" => $return, "title" => "Créer un service"]);
...
<option value="15">Instagram Auto Watch - Timed</option>
  ...
  $return .= '</select>
          ...
          ';
  echo json_encode(["content" => $return, "title" => "Créer un abonnement"]);
