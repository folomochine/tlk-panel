...
$_SESSION["toutlike_userlogin"]      = 1;
        $_SESSION["toutlike_userid"]         = $view["client_id"];
        $_SESSION["toutlike_userpass"]       = $view["password"];
  ...
  elseif( $action == "new" ):
      if( $_POST ):
        ...
        if( !email_check($email) ){
          ...
          $icon     = "error";
        }elseif( !empty($tel) && userdata_check("telephone",$tel) ){
          $error      = 1;
  ...
  elseif( $action == "edit" ):
    ...
    $client_detail  = getRow(["table"=>"clients","where"=>["client_id"=>$user_id]]);
    $client_access  = json_decode($client_detail["access"],true);
        if( $_POST ):
          ...
          $dupEmail = $conn->prepare("SELECT 1 FROM clients WHERE client_id != :uid AND email = :email LIMIT 1");
          $dupEmail->execute(["uid" => $user_id, "email" => $email]);
          $emailTaken = (bool) $dupEmail->fetchColumn();
          $phoneTaken = false;
          if ($tel !== '' && $tel !== null) {
            $dupPhone = $conn->prepare("SELECT 1 FROM clients WHERE client_id != :uid AND telephone = :tel LIMIT 1");
            $dupPhone->execute(["uid" => $user_id, "tel" => $tel]);
            $phoneTaken = (bool) $dupPhone->fetchColumn();
          }
  ...
  elseif( $action == "secret_category" ):
    ...
    $id     = $_GET["id"];
      if( $type == "on" ):
        $search = $conn->prepare("SELECT 1 FROM clients_category WHERE client_id = :client AND category_id = :cid LIMIT 1");
        $search->execute(["client" => $client, "cid" => $id]);
  ...
  elseif( $action == "secret_service" ):
    ...
    $id     = $_GET["id"];
      if( $type == "on" ):
        $search = $conn->prepare("SELECT 1 FROM clients_service WHERE client_id = :client AND service_id = :sid LIMIT 1");
        $search->execute(["client" => $client, "sid" => $id]);
        if( !$search->fetchColumn() ):
