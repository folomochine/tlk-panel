...
if ($methodId == 21) {
    $licenseKey = htmlspecialchars($_POST["license_key"]);
    $websiteId = htmlspecialchars($_POST["website_id"]);
    $exchangeRate = htmlspecialchars($_POST["exchange_rate"]);
    $exchangeRateValue = floatval($_POST["exchange_rate"]);
    if (empty($exchangeRateValue) || $exchangeRateValue == 0) {
        $exchangeRateValue = 1;
    }
    $methodExtras = [
...
if ($methodId == 21 && !empty($exchangeRateValue)) {
    $inverseRate = round(1 / $exchangeRateValue, 8);
    $syncCurrency = $conn->prepare("UPDATE currencies SET currency_rate=:rate, currency_inverse_rate=:inverse_rate WHERE currency_code=:code");
    $syncCurrency->execute([
