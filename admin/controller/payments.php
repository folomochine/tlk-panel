  ...
  if( $_SESSION["client"]["data"] ):
   ...
   if (route(2) == "bank"):
    ...
    elseif( route(2) == "online" ):
      ...
      $where          = ($page*$to)-$to;
      $paginationArr  = ["count"=>$pageCount,"current"=>$page,"next"=>$page+1,"previous"=>$page-1];
      $payments       = $conn->prepare("SELECT payments.*, payment_methods.*, clients.*, gateway_methods.methodId AS gateway_method_id, gateway_methods.methodFee AS gateway_method_fee, gateway_methods.methodCurrency AS gateway_method_currency FROM payments INNER JOIN payment_methods ON payment_methods.id=payments.payment_method INNER JOIN clients ON clients.client_id=payments.client_id LEFT JOIN paymentmethods AS gateway_methods ON gateway_methods.methodId=payments.payment_method $search ORDER BY payments.payment_id DESC LIMIT $where,$to ");
      $payments       -> execute(array());
      $payments       = $payments->fetchAll(PDO::FETCH_ASSOC);
