...
$modulesFlashKeys = ['success', 'successText', 'error', 'errorText'];
if ($_SESSION["client"]["data"]):
    $data = $_SESSION["client"]["data"];
    foreach ($modulesFlashKeys as $sk) {
...
function paymentStatu($statu) {
    ...
    if( $_GET["search_type"] == "username" && $_GET["search"] ):
      ...
      $clients      = $conn->prepare("SELECT client_id FROM clients WHERE username LIKE :sw ");
      $clients     -> execute(array("sw" => "%" . $search_word . "%"));
    ...
    elseif( $_GET["search_type"] == "payment_id" && $_GET["search"] ):
      $search_where = $_GET["search_type"];
      $search_word  = urldecode($_GET["search"]);
      $count        = $conn->prepare("SELECT * FROM payments INNER JOIN clients ON clients.client_id = payments.client_id WHERE CAST(payments.payment_id AS CHAR) LIKE :sw ");
      $count        -> execute(array("sw" => "%" . $search_word . "%"));
      $count        = $count->rowCount();
      $likePid = '%' . str_replace(['\\', '%', '_'], ['\\\\', '\\%', '\\_'], $search_word) . '%';
      $search       = "WHERE CAST(payments.payment_id AS CHAR) LIKE " . $conn->quote($likePid);
      $search_link  = "?search=".$search_word."&search_type=".$search_where;
    elseif( $_GET["search_type"] == "payment_amount" && $_GET["search"] ):
      $search_where = $_GET["search_type"];
      $search_word  = urldecode($_GET["search"]);
      $count        = $conn->prepare("SELECT * FROM payments INNER JOIN clients ON clients.client_id = payments.client_id WHERE CAST(payments.payment_amount AS CHAR) LIKE :sw ");
      $count        -> execute(array("sw" => "%" . $search_word . "%"));
      $count        = $count->rowCount();
      $likeUrl = '%' . str_replace(['\\', '%', '_'], ['\\\\', '\\%', '\\_'], $search_word) . '%';
      $search       = "WHERE orders.order_url LIKE " . $conn->quote($likeUrl) . " && orders.dripfeed='1' && orders.subscriptions_type='1' ";
      $search_link  = "?search=".$search_word."&search_type=".$search_where;
endif;
if ($_POST):
