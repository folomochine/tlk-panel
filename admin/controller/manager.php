...
$accessArray = json_encode($accessArray);
    ...
    if (empty($name) || strlen($name) < 5) {
        ...
        $icon     = "error";
    } elseif (!empty($telephone) && (function () use ($conn, $username, $telephone) {
        $st = $conn->prepare("SELECT 1 FROM admins WHERE username != :u AND telephone = :tel LIMIT 1");
        $st->execute(["u" => $username, "tel" => $telephone]);
...
elseif ($action == "new") :
    ...
    if (empty($name) || strlen($name) < 5) {
        ...
        $errorText  = "Please enter valid email format.";
        $icon     = "error";
    } elseif ((function () use ($conn, $username, $email) {
        $st = $conn->prepare("SELECT 1 FROM admins WHERE username != :u AND admin_email = :em LIMIT 1");
        $st->execute(["u" => $username, "em" => $email]);
        return (bool) $st->fetchColumn();
    })()) {
        $error      = 1;
        ...
        $errorText  = "The username must contain a minimum of 4 and a maximum of 32 characters, including letters and numbers..";
        $icon     = "error";
    } elseif ((function () use ($conn, $username) {
        $st = $conn->prepare("SELECT 1 FROM admins WHERE username = :u LIMIT 1");
...
elseif ($action == "username") :
    ...
    if (!username_check($username)) {
        ...
        $errorText  = "The username must contain a minimum of 4 and a maximum of 32 characters, including letters and numbers..";
        $icon     = "error";
    } elseif (($current_username != $username) && (function () use ($conn, $username) {
        $st = $conn->prepare("SELECT 1 FROM admins WHERE username = :u LIMIT 1");
        $st->execute(["u" => $username]);
        return (bool) $st->fetchColumn();
    })()) {
