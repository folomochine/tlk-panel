...
header("Location:".site_url("admin/$pageunits"));
  ...
  if( $_SESSION["client"]["data"] ):
    ...
    function orderStatu($statu,$error,$cron){
      if( $cron == "cronpending" ):
        $statu  = '<span style="display:inline-block;padding:4px 12px;border-radius:20px;font-size:11px;font-weight:600;background:rgba(234,179,8,.12);color:#eab308">En attente</span>';
      elseif( $error == "-" ):
        switch ($statu) {
          case 'pending':
            $statu  = '<span style="display:inline-block;padding:4px 12px;border-radius:20px;font-size:11px;font-weight:600;background:rgba(245,158,11,.12);color:#f59e0b">Pending</span>';
          break;
          case 'inprogress':
            $statu  = '<span style="display:inline-block;padding:4px 12px;border-radius:20px;font-size:11px;font-weight:600;background:rgba(59,130,246,.12);color:#3b82f6">En cours</span>';
          break;
          case 'completed':
            $statu  = '<span style="display:inline-block;padding:4px 12px;border-radius:20px;font-size:11px;font-weight:600;background:rgba(34,197,94,.12);color:#22c55e">Termin&eacute;e</span>';
          break;
          case 'partial':
            $statu  = '<span style="display:inline-block;padding:4px 12px;border-radius:20px;font-size:11px;font-weight:600;background:rgba(168,85,247,.12);color:#a855f7">Partielle</span>';
          break;
          case 'canceled':
            $statu  = '<span style="display:inline-block;padding:4px 12px;border-radius:20px;font-size:11px;font-weight:600;background:rgba(107,114,128,.12);color:#6b7280">Annul&eacute;e</span>';
          break;
          case 'processing':
            $statu  = '<span style="display:inline-block;padding:4px 12px;border-radius:20px;font-size:11px;font-weight:600;background:rgba(6,182,212,.12);color:#06b6d4">Traitement</span>';
          break;
        }
      else:
        $statu  = '<span style="display:inline-block;padding:4px 12px;border-radius:20px;font-size:11px;font-weight:600;background:rgba(239,68,68,.12);color:#ef4444">&Eacute;chou&eacute;</span>';
      endif;
      return $statu;
    }
