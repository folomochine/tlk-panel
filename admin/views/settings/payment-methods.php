...
<button type="button" class="btn btn-default m-b">Compte bancaire</button>
</a>
<div class="payment-methods-table">
    <table class="table">
        <thead>
            <tr>
                <th>
                    <span class="table__drag-header">Méthode</span>
                </th>
                <th>Nom visible</th>
                <th>Paiement min.</th>
                <th>Paiement max.</th>
        ...
        <tbody class="methods-sortable">
            <?php foreach($methodList as $method): $extra = json_decode($method["method_extras"],true); ?>
                <tr class="<?php if( $method["method_type"] == 1 ): echo "grey "; endif; ?>" data-toggle="<?php echo $method["id"]; ?>" data-id="<?php echo $method["id"]; ?>">
                    ...
                    <td class="p-r">
                        <button type="button" class="btn btn-primary btn-xs pull-right edit-payment-method" data-toggle="modal" data-target="#modalDiv" data-action="edit_paymentmethod" data-id="<?php echo $method["method_get"]; ?>">Modifier</button>
                    </td>
                </tr>
                <?php endforeach; ?>
        </tbody>
