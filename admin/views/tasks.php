      ...
      <li class="pull-right custom-search">
         <form class="form-inline" action="<?=site_url("admin/tasks")?>" method="get">
            <div class="input-group">
               <input type="text" name="search" class="form-control" value="<?=$search_word?>" placeholder="Rechercher une tache...">
...
<?php echo $order["service_name"]; ?></div></td>
                 ...
                 <td><?php if($order["task_type"] == 1): echo "<span class='badge badge-success'>RECHARGEMENT</span>"; elseif($order["task_type"] == 2): echo "<span class='badge badge-danger'>ANNULATION</span>";endif; ?></td>
                 <td><?php if($order["task_status"] == "failed"): echo "Echouee"; elseif($order["task_status"] == "inprogress"): echo "En cours"; elseif($order["task_status"] == "rejected"): echo "Rejetee";elseif($order["task_status"] == "completed"): echo "Terminee"; elseif($order["task_status"] == "canceled"): echo "Annulee";
                 endif; ?></td>
...
<li><a href="<?=site_url("admin/tasks/cancel_order/".$order["task_id"])?>">Annuler et rembourser</a></li>
<li>
<a href="#" class="check_seller_last_response" data-toggle="modal" data-action="task_id=<?=$order["task_id"]?>" data-target="#flipFlop">Derniere reponse du fournisseur</a></li>
<?php }elseif($order["task_type"] == 1){ ?>

<?php if($order["check_refill_status"] != 1){  ?>


<li><a href="<?=site_url("admin/tasks/update_refill_status/".$order["refill_orderid"])?>">Mettre a jour le statut</a></li>

<?php } ?>
<li><a href="#" class="check_seller_last_response" data-toggle="modal" data-action="task_id=<?=$order["task_id"]?>" data-target="#flipFlop">Derniere reponse du fournisseur</a></li>
