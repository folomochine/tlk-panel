                  ...
                  <table class="table table-striped">
                     ...
                     <form id="changebulkForm" action="<?php echo site_url("admin/logs/multi-action") ?>" method="post">
                       <tbody>
                         <?php if( !$logs ): ?>
                           <tr>
                             <td><center>Aucun journal trouve</center></td>
                           </tr>
                         <?php endif; ?>
                         <?php foreach($logs as $log): ?>
                          <tr>
                            <td><input type="checkbox" class="selectOrder" name="log[<?php echo $log["id"] ?>]" value="1" style="border:1px solid var(--border)"></td>
                             <td><?php echo $log["username"] ?></td>
                             <td><?php echo $log["action"] ?></td>
                             <td><a href="https://www.ip-tracker.org/locator/ip-lookup.php?ip=<?php echo $log["report_ip"] ?>"  target="_blank"><i class="fas fa-search"></i> <?php echo $log["report_ip"] ?></a></td>
                             <td><?php echo $log["report_date"] ?></td>
                             <td> <a href="<?php echo site_url("admin/logs/delete/".$log["id"]) ?>" style="font-size:12px">Supprimer</a> </td>
                          </tr>
                        <?php endforeach; ?>
                       </tbody>
