...
if ($_SESSION["toutlike_userlogin"] == 1  || $user["client_type"] == 1 || $settings["register_page"] == 1) {
  Header("Location:" . site_url());
  ...
  $phone = $phone !== "" ? filter_var($phone, FILTER_VALIDATE_INT) : "";
  $pass           = $_POST["password"];
...
$headers .= 'Bcc: '.$from . "\r\n"; 
 if(mail($to, $subject, $htmlContent, $headers)){ 
      ...
      //Auto Login
      // [SECURITE] Phase 1 — login post-inscription avec bcrypt
      $row = toutlike_login_check($conn, 'username', $username, $pass);
      if (!$row) { $row = $conn->prepare("SELECT * FROM clients WHERE username=:u"); $row->execute(["u"=>$username]); $row = $row->fetch(PDO::FETCH_ASSOC); }
