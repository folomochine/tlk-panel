...
$conn->beginTransaction();
  ...
  // [SECURITE] Phase 1 — vérification bcrypt pour changement de mot de passe
  if( !toutlike_verify_password($pass, $user["password"]) ){
    $error    = 1;
    ...
    $conn->beginTransaction();
      ...
      // [SECURITE] Phase 1 — nouveau mot de passe en bcrypt
      $new_hash = toutlike_hash_password($new_pass);
      $update = $conn->prepare("UPDATE clients SET password=:pass WHERE client_id=:id ");
      $update = $update->execute(array("id"=>$user["client_id"],"pass"=>$new_hash ));
        if( $update && $insert ):
          $_SESSION["toutlike_userpass"]       = $new_hash;
