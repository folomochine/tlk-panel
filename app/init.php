...
// [SECURITE] Phase 1 — cookies session sécurisés
...
if( !empty($_COOKIE["u_id"]) && !empty($_COOKIE["u_login"]) && !empty($_COOKIE["u_password"]) ):
...
if(!$_COOKIE["currency_hash"]){
    ...
    unset($_SESSION["toutlike_adminlogin"]);
    unset($_SESSION);
...
if( !empty($_COOKIE["a_id"]) && !empty($_COOKIE["a_login"]) && !empty($_COOKIE["a_password"]) ):
 ...
 $access   = json_decode($admin["access"],true);
  ...
  if( @$_COOKIE["a_password"] == $password ):
    ...
    unset($_SESSION["toutlike_adminlogin"]);
    unset($_SESSION["toutlike_adminid"]);
    unset($_SESSION["toutlike_adminpass"]);
    unset($_SESSION["toutlike_adminlogin"]);
    unset($_SESSION);
...
// [SECURITE] Phase 1 — autoescape activé pour protéger contre XSS
$twig     = new Twig_Environment($loader, ['autoescape' => 'html']);

// [SECURITE] Phase 1 — Rendre csrf_token et csrf_field disponibles dans Twig
$twig->addFunction(new Twig_SimpleFunction('csrf_token', 'csrf_token'));
$twig->addFunction(new Twig_SimpleFunction('csrf_field', 'csrf_field', ['is_safe' => ['html']]));
...
$user->execute(array("id"=> ($_SESSION["toutlike_userid"] ?? 0) ));
$user = $user->fetch(PDO::FETCH_ASSOC);
if (!$user) $user = [];
$user['auth']     = $_SESSION["toutlike_userlogin"] ?? 0;
if(  $user["auth"]  != 1):
...
$admin = $conn->prepare("SELECT * FROM admins WHERE admin_id=:id");
$admin->execute(array("id"=> ($_SESSION["toutlike_adminid"] ?? 0) ));
$admin = $admin->fetch(PDO::FETCH_ASSOC);
if (!$admin) $admin = [];
$admin['auth']     = $_SESSION["toutlike_adminslogin"] ?? 0;
...
if(!empty($user["currency_type"]) && $currency_code == $user["currency_type"]){
    $currencies_item .= '';
}else {
